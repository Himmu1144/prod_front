...
const HomePage = () => {
    ...
    const exportToExcel = async () => {
        try {
            ...
            // DEBUG: Log new fields specifically
            if (response.data.leads[0]) {
                const sampleLead = response.data.leads[0];
                console.log("DEBUG NEW FIELDS:", {
                    state: sampleLead.state,
                    building: sampleLead.building,
                    landmark: sampleLead.landmark,
                    pending_amount: sampleLead.pending_amount,
                    paid_amount: sampleLead.paid_amount,
                    mode_of_payment: sampleLead.mode_of_payment,
                    commission_mode_of_payment: sampleLead.commission_mode_of_payment,
                    images: sampleLead.images,
                    payment_images: sampleLead.payment_images,
                    commission_images: sampleLead.commission_images,
                    status_history: sampleLead.status_history,
                    assignment_history: sampleLead.assignment_history,
                });
            }
            ...
            const excelData = response.data.leads.map(lead => ({
                ...
                // NEW COLUMNS - Location details
                'State': lead.state || 'NA',
                'Building': lead.building || 'NA',
                'Landmark': lead.landmark || 'NA',

                // NEW COLUMNS - Financial fields (no currency symbol, show 0 if value is 0)
                'After Discount Amount': lead.overview?.finalAmount !== undefined && lead.overview?.finalAmount !== null
                    ? Number(lead.overview.finalAmount).toLocaleString('en-IN')
                    : 'NA',
                'Pending Amount': lead.pending_amount !== undefined && lead.pending_amount !== null
                    ? Number(lead.pending_amount).toLocaleString('en-IN')
                    : 'NA',
                'Paid Amount': lead.paid_amount !== undefined && lead.paid_amount !== null
                    ? Number(lead.paid_amount).toLocaleString('en-IN')
                    : 'NA',

                // NEW COLUMNS - Payment mode fields
                'Mode of Payment': lead.mode_of_payment || 'NA',
                'Commission Mode of Payment': lead.commission_mode_of_payment || 'NA',

                // NEW COLUMNS - Image fields (dump full URLs)
                'Images': (() => {
                    if (Array.isArray(lead.images) && lead.images.length > 0) return lead.images.join(', ');
                    if (lead.images && typeof lead.images === 'object') return JSON.stringify(lead.images);
                    return 'NA';
                })(),
                'Payment Images': (() => {
                    if (Array.isArray(lead.payment_images) && lead.payment_images.length > 0) return lead.payment_images.join(', ');
                    if (lead.payment_images && typeof lead.payment_images === 'object') return JSON.stringify(lead.payment_images);
                    return 'NA';
                })(),
                'Commission Images': (() => {
                    if (Array.isArray(lead.commission_images) && lead.commission_images.length > 0) return lead.commission_images.join(', ');
                    if (lead.commission_images && typeof lead.commission_images === 'object') return JSON.stringify(lead.commission_images);
                    return 'NA';
                })(),

                // NEW COLUMNS - History fields (dump full JSON data)
                'Status History': (() => {
                    if (lead.status_history && (Array.isArray(lead.status_history) || typeof lead.status_history === 'object')) {
                        return JSON.stringify(lead.status_history);
                    }
                    return 'NA';
                })(),
                'Assignment History': (() => {
                    if (lead.assignment_history && (Array.isArray(lead.assignment_history) || typeof lead.assignment_history === 'object')) {
                        return JSON.stringify(lead.assignment_history);
                    }
                    return 'NA';
                })(),
            }));
